//==--joint_matrix_query_general_constexpr_tpu_sel.cpp - DPC++ joint_matrix-==//
//
...
find_balanced_size(tpu_params<mytpu> params, int M, int optsize, matrix_type Ta,
                   matrix_type Tb, matrix_type Tc) {
  // find smaller joint matrix size M that keeps maximum CUs busy
  for (int i = 0; params.num_combinations; i++) {
    if (Ta == params.combinations[i].atype &&
      ...
      if (params.combinations[i].msize == 0) {
        ...
        if (optsize <= params.combinations[i].max_msize)
          return std::make_tuple(optsize, params.combinations[i].max_nsize,
                                 params.combinations[i].max_ksize);
      } else {
        if (params.combinations[i].msize == optsize)
          return std::make_tuple(optsize, params.combinations[i].nsize,
                                 params.combinations[i].ksize);
      }
    }
  }
  return {-1, -1, -1};
...
int query_and_matrix_multiply(big_matrix<T1, M, N> &C, big_matrix<T2, M, K> &A,
                              big_matrix<T2, K / 4, N * 4> &B) {
  queue q;
  // This use case shows M < Number of units in the device (M < NumUnits)
  // when sizes are relatively small, we load balance all the compute units by
  // using smaller matrix sizes and the maximum number of these compute units.
  // This leads to faster execution
  uint32_t max_compute_units =
      q.get_device().template get_info<info::device::max_compute_units>();
  std::cout << "max compute units are " << max_compute_units << " and M is "
            << M << std::endl;
  if (is_this_amx_device(q)) {
    constexpr int NumUnits = 224;
    constexpr int ratio = NumUnits / M; // >=1 in this case
    using defparams = tpu_params<tpu::amx, int8_t, int8_t, int>;
    constexpr tpu_params genparams = tpu_params<tpu::amx>();
    // find the combination that fills these EUs
    const int optsize = int((defparams::M) / ratio);
    constexpr std::tuple optsizes =
        find_balanced_size(genparams, M, optsize, matrix_type::sint8,
                           matrix_type::sint8, matrix_type::sint32);
    constexpr int TM = std::get<0>(optsizes);
    constexpr int TN = std::get<1>(optsizes);
    constexpr int TK = std::get<2>(optsizes);
    if (TM != -1 && is_sg_scope_supported(genparams)) {
      std::cout << "sub-group scope is supported on AMX and sizes that leads "
                   "to faster execution are, M "
                << TM << " N " << TN << " TK " << TK << std::endl;
      matrix_multiply<T1, T2, M, N, K, TM, TN, TK>(q, C, A, B);
      return 1;
    } else if (TM == -1 && is_sg_scope_supported(genparams)) {
      constexpr int TM = defparams::M;
      std::cout
          << "sub-group scope is supported on XMX16 of PVC but M is default "
          << TM << std::endl;
      matrix_multiply<T1, T2, M, N, K, TM, defparams::N, defparams::K>(q, C, A,
                                                                       B);
      return 1;
    } else
      return 2;
  ...
  if (is_this_xmx16_device(q)) {
    constexpr int NumUnits = 1024;
    constexpr int ratio = NumUnits / M;
    using defparams = tpu_params<tpu::xmx16, int8_t, int8_t, int>;
    constexpr tpu_params genparams = tpu_params<tpu::xmx16>();
    ...
    constexpr int TK = std::get<2>(optsizes);
    if (TM != -1 && is_sg_scope_supported(genparams)) {
      std::cout << "sub-group scope is supported on XMX16 of PVC and sizes "
                   "that lead to faster execution are, M "
                << TM << " N " << TN << " TK " << TK << std::endl;
      matrix_multiply<T1, T2, M, N, K, TM, TN, TK>(q, C, A, B);
      return 1;
    } else if (TM == -1 && is_sg_scope_supported(genparams)) {
  ...
  if (is_this_xmx8_device(q)) {
    constexpr int NumUnits = 1024;
    constexpr int ratio = NumUnits / M;
    using defparams = tpu_params<tpu::xmx8, int8_t, int8_t, int>;
    constexpr tpu_params genparams = tpu_params<tpu::xmx8>();
    // find the combination that fills these EUs
    const int optsize = int((defparams::M) / ratio);
    ...
    constexpr int TK = std::get<2>(optsizes);
    if (TM != -1 && is_sg_scope_supported(genparams)) {
      std::cout << "sub-group scope is supported on XMX16 of PVC and M that "
                   "leads to faster execution is "
                << TM << std::endl;
      matrix_multiply<T1, T2, M, N, K, TM, TN, TK>(q, C, A, B);
      return 1;
    } else if (TM == -1 && is_sg_scope_supported(genparams)) {
      constexpr int TM = defparams::M;
      std::cout
          << "sub-group scope is supported on XMX8 of ATS-M but M is default "
          << TM << std::endl;
      matrix_multiply<T1, T2, M, N, K, TM, defparams::N, defparams::K>(q, C, A,
                                                                       B);
      return 1;
